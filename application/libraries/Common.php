...
{ 
	...
	public function ExecuteSMSApiWhatsapp($mobile_no,$message)
	{
		$CI =& get_instance();
		$url2 = 'https://whatsbot.tech/api/send_sms?api_token=&mobile=91'.$mobile_no.'&message='.urlencode($message);
		//echo $url2;exit;
		$ch = curl_init();
		...
		$CI->load->model('Sms');
		curl_close($ch);
		$CI->Sms->addSentMessage($mobile_no,$message,"Whatsapp:".$buffer);
		return $buffer;
	}
