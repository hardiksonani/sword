						...
						if($ip == $client_ip or $ip == $client_ip2 or $ip == $client_ip3 or $ip == $client_ip4)
						{
							if($status == '1')
							{
								...
								if(ctype_digit($sendermobile))
								{
										...
										$rsltsender = $this->db->query("select Id,mobile,name,lastname,pincode from remitters where mobile = ? and insta_remitter_id > 0 order by Id desc limit 1",array($sendermobile));
										if($rsltsender->num_rows() == 1)
										{
											$checkbeneexist = $this->db->query("select * from beneficiaries 
            														where sender_mobile = ? and insta_bene_id  = ? and status = 'SUCCESS' 
            														order by Id desc limit 1 ",array(
            														$sendermobile,$bene_id));
											if($checkbeneexist->num_rows() > 0)
											{
											    $bank_id = $checkbeneexist->row(0)->dezire_bank_id;
											    $RESP_beneficiary_id = $checkbeneexist->row(0)->Id;
													...
													$resparray = array();
														...
														if($userinfo_balance->num_rows() == 1)
														{
															if(floatval($userinfo_balance->row(0)->balance) > $amount)
															{
																   ...
																   echo  $this->Instapay->transfer2($sendermobile,$beneficiaryid,$amount,$mode,$userinfo,0,"API",$bank_id,$amount,$order_id);
																   //($sendermobile,$beneficiary_array,$amount,$mode,$userinfo,$order_id,0);exit;	
