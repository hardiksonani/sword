...
class GetSenderRegisterOtp extends CI_Controller 
{
		...
		if(isset($header_array["Authkey"]))
		{
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST')
			{
				...
				if(isset($json_obj->username) && isset($json_obj->password) && isset($json_obj->sendermobile) )
				{
					...
					$sendername = trim($json_obj->sendername);
					$senderlastname = trim($json_obj->senderlastname);
						...
						if($ip == $client_ip or $ip == $client_ip2 or $ip == $client_ip3 or $ip == $client_ip4)
						{
							if($status == '1')
							{
								...
								if(ctype_digit($sendermobile))
								{
										...
										echo $this->Instapay->remitter_registration2($sendermobile,$sendername,$senderlastname,$pincode,$userinfo);
								}
								else
								{
									$resp_array = array(
