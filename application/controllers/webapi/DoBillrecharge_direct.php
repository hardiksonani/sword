...
class DoBillrecharge_direct extends CI_Controller 
{
	...
	{ 
		//https://www.mastermoney.in/webapi/doBillrecharge?username=&pwd=&mcode=&serviceno=&customer_mobile=&option1=&Amount=
		...
		if($_SERVER['REQUEST_METHOD'] == 'GET')
		{
			...
			//	billpay($userinfo,$spkey,$company_id,$Amount,$Mobile,$CustomerMobile,$remark,$RefId,$option1,$option2="",$option3="",$done_by = "WEB")
				...
				{
					$company_info = $this->db->query("select company_id from tblcompany where mcode = ?",array($mcode));
					...
					if($company_info->num_rows() == 1)
					{
						...
						if($balance >= $Amount)
						{
        						...
        						$response = $this->Dezire->fetchbill($user_info,$mcode,$company_info->row(0)->company_id,$serviceno,$customer_mobile,$option1);
						
						
							
							$response = $this->Dezire->recharge_transaction2($user_info,$mcode,$company_info->row(0)->company_id,$Amount,$serviceno,$customer_mobile,$remark,$option1,$RefId,$particulars);
							//($user_info,$company_info->row(0)->billerId,$Amount,$serviceno,$customer_mobile,$remark,$RefId,$option1);

							//getbillFetch($company_info->row(0)->billerId,$customer_mobile,$serviceno,$option1,$option2,$user_info);
							echo $response;exit;
						}
						else
						{
