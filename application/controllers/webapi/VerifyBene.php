	...
	{ 
		...
		if(isset($header_array["Authkey"]))
		{
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST')
			{
				...
				if(isset($json_obj->username) && isset($json_obj->password) && isset($json_obj->sendermobile) && isset($json_obj->account_number) && isset($json_obj->ifsc) && isset($json_obj->bank_id) )
				{
					$username = trim($json_obj->username);
					$pwd =  trim($json_obj->password);
					$sendermobile = trim($json_obj->sendermobile);
					$account_number = trim($json_obj->account_number);
					$ifsc = trim($json_obj->ifsc);
						...
						if($ip == $client_ip or $ip == $client_ip2 or $ip == $client_ip3 or $ip == $client_ip4)
						{
							if($status == '1')
							{
								...
								if(ctype_digit($sendermobile))
								{
										...
										$rsltsender = $this->db->query("select Id,mobile,name,lastname,pincode from remitters where mobile = ? and insta_remitter_id > 0 order by Id limit 1",array($sendermobile));
										if($rsltsender->num_rows() == 1)
										{
											//echo $sendermobile."  ".$account_number."  ".$ifsc;exit;
											$this->load->model("Instapay");
											echo $this->Instapay->verify_bene($sendermobile,$account_number,$ifsc,$bank_id,$userinfo,0);exit;
										}
										else
										{
											
											$this->load->model("Instapay");
											echo $this->Instapay->verify_bene2($sendermobile,$account_number,$ifsc,$bank_id,$userinfo,0);exit;
