	...
	{ 
		...
		if(isset($header_array["Authkey"]))
		{
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST')
			{
				...
				if(isset($json_obj->username) && isset($json_obj->password) && isset($json_obj->sendermobile) && isset($json_obj->otp) )
				{
						...
						if($ip == $client_ip or $ip == $client_ip2 or $ip == $client_ip3 or $ip == $client_ip4)
						{
							if($status == '1')
							{
								...
								if(ctype_digit($sendermobile))
								{
										...
										echo $this->Instapay->remitter_resend_otp2($sendermobile,$otp,$userinfo,$firstname,$lastname,$pincode);
								}
								else
								{
									$resp_array = array(
