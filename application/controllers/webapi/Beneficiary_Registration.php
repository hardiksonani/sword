...
class Beneficiary_Registration extends CI_Controller 
{
	...
	{ 
		...
		if(isset($header_array["Authkey"]))
		{
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST')
			{
				...
				if(isset($json_obj->username) && isset($json_obj->password) && isset($json_obj->sendermobile) && isset($json_obj->benificiary_name) && isset($json_obj->benificiary_ifsc) && isset($json_obj->benificiary_account_no) && isset($json_obj->benificiary_mobile) && isset($json_obj->bank_id) )
				{
						...
						if($ip == $client_ip or $ip == $client_ip2 or $ip == $client_ip3 or $ip == $client_ip4)
						{
							if($status == '1')
							{
								...
								if(ctype_digit($sendermobile))
								{
										...
										$rsltsender = $this->db->query("select Id,mobile,name,lastname,pincode from remitters where mobile = ? and otp_varified = 'yes' order by Id desc limit 1",array($sendermobile));
										//print_r($rsltsender->result());
										if($rsltsender->num_rows() == 1)
										{
											
											$this->load->model("Instapay");
											echo $this->Instapay->beneficiary_register($sendermobile,$benificiary_name,$benificiary_mobile,$benificiary_ifsc,$benificiary_account_no,$userinfo,$bank_id = 0);
											//($sendermobile,$benificiary_name,$benificiary_mobile,$benificiary_account_no,$benificiary_ifsc,$bank_id,$userinfo);
										}
										else
										{
