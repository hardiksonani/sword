...
class Recharge_home extends CI_Controller 
{
    ...
    <tbody><tr class="operator-showoptcomm">
		...
		select 
		    ...
		    where   a.service_id = ?  order by a.service_id,a.company_name",array($this->session->userdata("MdId"),$service_id));
	...
	public function viewbill()
	{
		...
		{
			$userinfo = $this->db->query("select * from tblusers where user_id = ?",array($this->session->userdata("MdId")));
			...
			if(true)
			{
				...
				<div class="body table-responsive" style="position:inherit;">
					<table id="example" class="table table-bordered example-recharge" cellspacing="0" style="width:100%;">
						...
						<tbody>';
//echo $this->session->userdata("MdId");exit;
				$lasttxns = $this->db->query("select a.recharge_id,a.mobile_no,a.amount,a.operator_id,a.recharge_status,a.add_date,b.company_name from tblrecharge a
                    left join tblcompany b on a.company_id = b.company_id where a.user_id = ? and b.service_id = ? order by a.recharge_id desc limit 10",array($this->session->userdata("MdId"),intval($service_id)));
...
txtAmount: 1500
	...
	{	
		...
		$this->output->set_header("Pragma: no-cache"); 
			...
			{		
				...
				$this->load->view('MasterDealer/recharge_home_view',$this->view_data);
			}
	...
	{
		$mob = $_GET["mob"];
		$rslt = $this->db->query("select custname from tblrecharge where mobile_no = ? and user_id = ? order by recharge_id desc limit 1",array($mob,$this->session->userdata('MdId')));
		if($rslt->num_rows() > 0)
		{
