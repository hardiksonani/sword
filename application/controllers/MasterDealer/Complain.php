...
class Complain extends CI_Controller {
	...
	public function pageview()
	{
		...
		$result = $this->db->query("select complain_id from tblcomplain where user_id = ?",array($this->session->userdata("MdId")));
		$total_row = $result->num_rows();		
		$this->load->library('pagination');
		$config['base_url'] = base_url()."MasterDealer/complain/pageview";
		$config['total_rows'] = $total_row;
		$config['per_page'] = $per_page; 
		$this->pagination->initialize($config); 
		$this->view_data['pagination'] = $this->pagination->create_links();
		$this->view_data['result_complain'] = $this->db->query("select * from tblcomplain where user_id = ? order by complain_id desc limit ?,?",array($this->session->userdata("MdId"),$start_row,$per_page));
		$this->view_data['message'] =$this->msg;
		$this->view_data['cmpl_flag'] =0;
		$this->load->view('MasterDealer/complain_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
		...
		if(isset($_POST["txt_frm_date"]) and isset($_POST["txt_to_date"]))
		{
			...
			",array($this->session->userdata("MdId"),$from,$to));
		...
		else
		{
			...
			,a.user_id,a.complain_date,a.complainsolve_date,a.message,a.response_message,a.complain_status FROM `tblcomplain` a left join tblrecharge b on a.recharge_id = b.recharge_id
			left join tblcompany d on b.company_id = d.company_id 
			where a.user_id = ?",array($this->session->userdata("MdId")));
