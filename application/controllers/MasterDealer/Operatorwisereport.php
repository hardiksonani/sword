...
class Operatorwisereport extends CI_Controller {
	...
	{	
		...
		$this->view_data['result_all'] = $this->get_recharge($from,$to,$this->session->userdata("MdId"));
		$this->view_data['message'] =$this->msg;
		$this->load->view('MasterDealer/operatorwisereport_view',$this->view_data);			
...
$this->output->set_header("Pragma: no-cache"); 
	...
	private function get_recharge($start_date,$end_date,$user_id)
	{
		...
		$result = $this->db->query("
				...
				IFNULL(Sum(a.MDComm),0) as DComm 
				from tblrecharge a 
			...
			",array($start_date,$end_date,$this->session->userdata("MdId")));
