...
class Topuphistory extends CI_Controller {
	...
	//	echo $start_row;exit;
		...
		$config['base_url'] = base_url()."Retailer/Topuphistory/pageview";
		...
		$totalcr = 0;
		$totaldr = 0;
		$topup_summary = $this->db->query("
		                            select 
    		                            IFNULL(Sum(credit_amount),0) as totalcr,
    		                            IFNULL(Sum(debit_amount),0) as totaldr 
    		                            from tblewallet 
    		                            where 
    		                            debit_amount > 0 and
    		                            ...
    		                            ",array($user_id,$fromdate,$todate));
		
		
		if($topup_summary->num_rows() == 1)
		{
		    $totalcr = $topup_summary->row(0)->totalcr;
		    $totaldr = $topup_summary->row(0)->totaldr;
		}
		
		
		
		
		$this->view_data['total_credit'] =round($totalcr,2); 
		$this->view_data['total_debit'] =round($totaldr,2); 
		$rows = $this->AccountLedger_getReport($user_id,$fromdate,$todate,$ddldb,$start_row,$per_page,$ddluser);
		

		$this->view_data['ddluser'] = $ddluser;
		$this->view_data['result_all'] = $rows;
		$this->view_data['message'] =$this->msg;
		$this->load->view('MasterDealer/topuphistory_view',$this->view_data);			
	}
...
$this->output->set_header("Pragma: no-cache"); 
	...
	private function AccountLedger_getReport($user_id,$from_date,$to_date,$ddldb,$start_row,$per_page,$ddluser)
	{
    		...
    		//echo $from_date."   ".$to_date;exit;
    			...
    			a.user_id = ? and 
    			if(? > 0,pay.cr_user_id = ?,true) and

    			DATE(a.add_date) >= ? and 
    			DATE(a.add_date) <= ?
    			order by a.Id desc limit ?,? ";
        		$rslt = $this->db->query($str_query,array($user_id,$ddluser,$ddluser,$from_date,$to_date,intval($start_row),intval($per_page)));
		...
		if(isset($_GET["from"]) and isset($_GET["to"]))
		{
			...
			$data = array();
				...
				left join tblusers p on cr.parentid = p.user_id
				 ...
				 Date(a.add_date) <= ?
				 order by Id";
		$rslt = $this->db->query($str_query,array($this->session->userdata("MdId"),$from,$to));
    ...
    $fileName = "Balance Topup Report From ".$from." To  ".$to.".xls";
