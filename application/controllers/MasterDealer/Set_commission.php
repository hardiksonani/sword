...
class Set_commission extends CI_Controller {
	...
	function getCommission()
	{
	    return "";
			...
			if($company_info->num_rows() == 1)
			{
				...
				$dist_commissioninfo = $this->getCommissionInfo($company_id,$this->session->userdata("MdId"),$this->session->userdata("DistSchemeId"));
				...
				foreach($userinfo->result() as $rwuser)
				{
					...
					$fos_commissioninfo = $this->getCommissionInfo($company_id,$rwuser->fos_id,$rwuser->fos_scheme_id);
					$fos_commissionrange = $this->getCommissionRange($company_id,$rwuser->fos_id,$rwuser->fos_scheme_id);
					
					$agent_commissioninfo = $this->getCommissionInfo($company_id,$rwuser->user_id,$rwuser->scheme_id);
					$agent_commissionrange = $this->getCommissionRange($company_id,$rwuser->user_id,$rwuser->scheme_id);
					
					$commission_left = $dist_commissioninfo["Commission_Per"] - $fos_commissioninfo["Commission_Per"] - $agent_commissioninfo["Commission_Per"];
					
					$str .='<tr>';
						$str .='<td>'.$i.'</td>';
						$str .='<td>'.$dist_commissioninfo["Commission_Per"].'</td>';
						$str .='<td>'.$commission_left.'</td>';
						$str .='<td>'.$rwuser->fos_name.'</td>';
						$str .='<td>'.$fos_commissionrange["min_com_limit"].' - '.$fos_commissionrange["max_com_limit"].'</td>';
						$str .='<td>
						<input type="text" class="form-control" id="txtFosComm'.$rwuser->user_id.'" value="'.$fos_commissioninfo["Commission_Per"].'"></td>';
						...
						$str .='<td>
						<input type="button" class="btn btn-success btn-mini" id="btnsubmit'.$rwuser->user_id.'" value="Save" onClick="changecommission('.$rwuser->user_id.','.$rwuser->fos_id.')"></td>';
	...
	function ChangeCommission()
	{
		...
		$foscom = trim($_GET["foscom"]);
		$user_id = trim($_GET["user_id"]);
		$fos_id = trim($_GET["fos_id"]);
				...
				if($userinfo->row(0)->usertype_name == "Agent")
				{
					...
					if($com <= $parent_commission_per)
					{
						if($com <= $max_com_limit and $com >= $min_com_limit)
						{
							...
							$fosuserinfo = $this->db->query("select * from tblusers where user_id = ? and parentid = ?",array($fos_id,$this->session->userdata("MdId")));
							if($fosuserinfo->num_rows() == 1)
							{
								if($fosuserinfo->row(0)->usertype_name == "FOS")
								{
									$parentinfo = $this->db->query("select * from tblusers where user_id = ?",array($fosuserinfo->row(0)->parentid));
									$this->load->model("Do_recharge_model");
									

									$fos_commission_range = $this->Do_recharge_model->getCommissionRange($company_id,$fosuserinfo);
									$fos_min_com_limit =$fos_commission_range["min_com_limit"];
									$fos_max_com_limit =$fos_commission_range["max_com_limit"];

									
									if($foscom <= $parent_commission_per)
									{
										if($foscom <= $fos_max_com_limit and $foscom >= $fos_min_com_limit)
										{
											$comtype = $parent_commission_type;
											$check_rslt = $this->db->query("select * from tbluser_commission where user_id = ? and company_id = ?",array($fos_id, $company_id));
											if($check_rslt->num_rows()  >= 1)
											{
												$rslt = $this->db->query("update tbluser_commission set commission = ?,commission_type = ? where Id = ?",array($foscom,$comtype,$check_rslt->row(0)->Id));
												echo "OK";exit;
											}
											else
											{
												$add_date = $this->common->getDate();
												$str_qry = "insert into tbluser_commission(user_id,company_id,commission,commission_type,min_com_limit,max_com_limit,add_date) values(?,?,?,?,?,?,?)";
												$rslt_in = $this->db->query($str_qry,array($fos_id,$company_id,$foscom,$comtype,0,0,$add_date));
												echo "OK";exit;
											}
										}
										else
	...
	function getdealer()
	{
		...
		$id = $_GET["id"];
		$rsltdealer = $this->db->query("select * from tblusers where parentid = ? and usertype_name = 'MasterDealer'",array($id));
