...
class Change_password extends CI_Controller {
	...
	public function index() 
	{
		...
		if(isset($_POST["Item1_OldPassword"]) and isset($_POST["Item1_NewPassword"]) and isset($_POST["Item1_ConfirmPassword"]))
		{
		   // print_r($this->input->post());exit;
		    /*
		    Array ( [hidID] => [txtOldPassword] => 123456 [txtNewPassword] => 123456 [txtCNewPassword] => 123456 [btnSubmit] => Submit )
		    */
			$oldPwd = trim($this->input->post("Item1_OldPassword",TRUE));
			$newPwd = $this->input->post("Item1_NewPassword",TRUE);
			$newCPwd = $this->input->post("Item1_ConfirmPassword",TRUE);
			
			if($newPwd == $newCPwd)
			{
			    if(strlen($newPwd) <= 20 and strlen($newPwd) >= 4)
    			{
    			    $user_id = $this->session->userdata("MdId",TRUE);
