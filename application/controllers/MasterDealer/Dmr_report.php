...
$this->output->set_header("Pragma: no-cache"); 
		...
		else if($this->input->post('txt_frm_date'))
		{
...
Date(a.add_date) <= ? and
if(? > 0,a.user_id = ?,true) 
order by Id desc ",array($user_id,$from,$to,$ddluser,$ddluser));
			...
			$this->view_data["summary_array"] = $this->getTotalDMRValues($from,$to,$ddluser);
...
left join tblusers dist on b.parentid = dist.user_id
left join dezire_banklist bank on a.bank_id = bank.Id
where dist.parentid = ? and Date(a.add_date) >= ? and Date(a.add_date) <= ? order by Id desc",array($user_id,$from,$to));
					$this->view_data['message'] =$this->msg;
					$this->view_data['from'] =$from;
					$this->view_data['to'] =$to;
			...
			$this->view_data['txtRemitter'] ="";
					...
					$this->view_data["summary_array"] = $this->getTotalDMRValues($from,$to,0);
					$this->load->view('MasterDealer/dmr_report_view',$this->view_data);		
				}
				else
				{redirect(base_url().'login');}																								
	...
	private function getTotalDMRValues($from,$to,$ddluser)
	{
	    $user_id = $this->session->userdata("MdId");
		$total_success = 0;
		$total_failure = 0;
		...
		$rslt = $this->db->query("
			...
			if(? > 0,a.user_id = ?,true)
			 ...
			 group by Status",array($from,$to,$user_id,$ddluser,$ddluser));
