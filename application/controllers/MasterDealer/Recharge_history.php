...
class Recharge_history extends CI_Controller {
	...
	//echo "asdfsdf";exit;
		...
		$config['base_url'] = base_url()."MasterDealer/recharge_history/commonfunction";
        ...
        $this->db->reconnect();
			...
			$rslt_sf = $this->db->query("SELECT 
										...
										a.MdId = ? and
										if(? > 0,a.user_id = ?,true)
										 group by a.recharge_status ",array($from,$to,$this->session->userdata("MdId"),$ddluser,$ddluser));
		...
		$this->view_data['result_all'] = $this->get_recharge($from,$to,$this->session->userdata("MdId"),$start_row,$per_page,$word,$ddldb,$ddlstatus,$ddloperator,$ddluser);
		$this->view_data['message'] =$this->msg;
		$this->load->view('MasterDealer/recharge_history_view',$this->view_data);			
	...
	private function get_recharge($start_date,$end_date,$user_id,$startrow,$perpage,$word,$ddldb,$ddlstatus,$ddloperator,$ddluser)
	{
		...
		{ 
			...
			$str_query ="
				...
				if(? != '',b.mobile_no = ?,true) 
				order by a.Id desc";		
				$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("MdId"),$ddluser,$ddluser,$ddlstatus,$ddlstatus,$ddloperator,$ddloperator,$word,$word));
				return $result;
