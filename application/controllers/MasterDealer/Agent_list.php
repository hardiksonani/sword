...
class Agent_list extends CI_Controller {
	...
	public function pageview()
	{
		...
		$config['base_url'] = base_url()."MasterDealer/agent_list/pageview";
		...
		$this->load->view('MasterDealer/agent_list_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
	...
	public function Search($SearchBy,$SearchWord)
	{
		...
		left join tblusers p on a.parentid = p.user_id
		where 
		a.usertype_name = 'Distributor' and a.parentid = ? and
		...
		left join tblusers p on a.parentid = p.user_id
		where 
		a.usertype_name = 'Distributor' and and a.parentid = ? and
	...
	public function dataexporttwo()
	{
		...
		where 
		a.usertype_name = 'Distributor'  and a.parentid = ?
		order by a.businessname",array($this->session->userdata("MdId")));
	...
	public function getbalance()
	{
		...
		{
			$id = $_GET["id"];
	   		echo $id."#".round($this->Common_methods->getAgentBalance($id),2);exit;
		}
		else
		{
			echo round($this->Common_methods->getAgentBalance($this->session->userdata("MdId")),2);
		}
