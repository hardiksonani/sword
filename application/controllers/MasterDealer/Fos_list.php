...
class Fos_list extends CI_Controller {
	...
	public function pageview()
	{
		...
		$userrows = $this->db->query("select count(user_id) as total from tblusers where usertype_name = 'FOS' and parentid = ?",array($user_id));
		
		$total_row = $userrows->row(0)->total;
		$this->load->library('pagination');
		$config['base_url'] = base_url()."MasterDealer/fos_list/pageview";
		...
		where 
		a.usertype_name = 'FOS'  and a.parentid = ?
		...
		$this->load->view('MasterDealer/fos_list_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
	...
	public function Search($SearchBy,$SearchWord)
	{
		...
		left join tblusers p on a.parentid = p.user_id
		where 
		a.usertype_name = 'FOS' and a.parentid = ? and
		...
		left join tblusers p on a.parentid = p.user_id
		where 
		a.usertype_name = 'FOS' and and a.parentid = ? and
	...
	public function getbalance()
	{
		...
		$id = $_GET["id"];
	   echo $id."#".$this->Common_methods->getAgentBalance($id);exit;
