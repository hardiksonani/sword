       ...
       // $this->logentry("","",json_encode($this->input->get()));
		...
		if($_SERVER['REQUEST_METHOD'] == 'GET') 
		{
			if(isset($_GET['username']) && isset($_GET['password']) && isset($_GET['remittermobile']) && isset($_GET['bene_id']) && isset($_GET['amount']) && isset($_GET['mode'])  && isset($_GET['bank_id']))
			{
				...
				$userinfo = $this->db->query("select user_id,businessname,username,status,usertype_name,mobile_no,parentid,mt_access,txn_password,password from tblusers where username = ?  and password = ?",array($username,$pwd));
