...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$this->view_data['pagination'] = NULL;
				    ...
				    if($ddlstatus == "PENDING" or $ddlstatus == "HOLD")
    				{
					
    				    $this->view_data['result_all'] = $this->db->query("SELECT a.unique_id,a.retry,a.API,a.Id,a.order_id,
    				    a.ccf,a.cashback,a.tds,
    				    a.add_date,a.edit_date,a.user_id,a.Charge_type,a.charge_value,a.Charge_Amount,a.RemiterMobile,
    ...
    left join tblusers b on a.user_id = b.user_id
     ...
     if(? != 'ALL',a.API = ?, true) 
     order by Id desc",array($from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo,$ddlapi,$ddlapi));
    				}
    				else
    				{
    ...
    left join dezire_banklist bank on a.bank_id = bank.Id
     ...
     order by Id desc limit 50",array($from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo,$ddlapi,$ddlapi));    
				...
				$this->load->view('_Admin/dmr_report_view',$this->view_data);		
			}	
 ...
 where a.Id = ? order by Id desc",array($hidId));
					...
					if($rslttxncheck->num_rows() == 1)
					{
						...
						else if(($Status == "PENDING" or  $Status == "HOLD" or  $Status == "SUCCESS") and $hidstatus == "Failure")
						{
							...
							$paymentinfo = $this->db->query("SELECT transaction_type,description,remark,credit_amount,debit_amount FROM tblewallet where dmr_id =? and user_id = ?
UNION
SELECT transaction_type,description,remark,credit_amount,debit_amount FROM jantrech_archive.tblewallet where dmr_id =? and user_id = ?",array($dmr_id,$user_id,$dmr_id,$user_id));

                           // print_r($paymentinfo->num_rows());exit;
							if($paymentinfo->num_rows() >= 1)
							{
							   ...
							   // echo $difference;exit;
								if($difference == $Amount)
								{
									...
									$this->Mastermoney->PAYMENT_CREDIT_ENTRY($user_id,$dmr_id,$transaction_type,$Amount,$Description,$sub_txn_type,$remark,$chargeAmount);
								}
						...
						{
							$this->db->query("update mt3_transfer set Status = 'SUCCESS' where Id = ?",array($dmr_id));
							redirect(base_url()."_Admin/dmr_report?crypt=".$this->Common_methods->encrypt("MyData"));
						}
...
FROM `mt3_transfer` a where a.Id = ? order by Id desc",array($Id));
			if($result->num_rows() == 1)
			{
        		...
        		else
        		{
        		    ...
        		    if($result->row(0)->API == "MASTERMONEY")
        		    {
        		        $this->load->model("Mastermoney");
				        echo $this->Mastermoney->transfer_status($Id);exit;        
        		    }
