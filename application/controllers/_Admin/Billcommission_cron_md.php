...
$this->output->set_header("Pragma: no-cache"); 
       ...
       // $transaction_date = '2019-03-21';
        ...
        left join tblusers m on p.parentid = m.user_id
        where 
        Date(a.add_date) = ? and
        b.usertype_name = 'Agent' and
        a.status = 'Success'  and
        a.bill_amount < 100000 and
        m.user_id != 29 and
        m.user_id != 2408
    	...
    	foreach($rsltuser->result() as $rw)
    	{
		    ...
		    if($host_id == 4381)
		    {
		        $commamount = (($rw->total * 0.30) / 100);    
		        $description = "Bill Payment Comm.:(".$rw->total." * 0.30 ) / 100,";
		    }
		    else
		    {
		        $commamount = (($rw->total * 0.10) / 100);
		        $description = "Bill Payment Comm.:(".$rw->total." * 0.10 ) / 100,";
		    }
    		...
    		else
    		{
    		    ...
    		    //if(true)
    		    {
        		    ...
        		    $remark = "BILL Payment Commission";
        		     if($host_id == 4381)
        		     {
        		        $description = "BillComm.Date:".$transaction_date .",(".$rw->total." * 0.30 /100)";     
        		     }
        		     else
        		     {
        		         $description = "BillComm.Date:".$transaction_date .",(".$rw->total." * 0.10 /100)";
        		     }
