...
class Group extends CI_Controller {
	...
	public function pageview()
	{
		...
		$result = $this->db->query("select count(Id) as total from tblgroup");
		...
		$this->view_data['result_group'] = $this->db->query("select a.*,b.businessname,b.usertype_name from tblgroup a left join tblusers b on a.user_id = b.user_id  order by a.group_name limit ?,?",array($start_row,$per_page));
...
$this->output->set_header("Pragma: no-cache"); 
		...
		//	print_r($this->input->post());exit;
			if($this->input->post("HIDACTION") == "INSERT")
			{
				...
				$check = $this->db->query("select * from tblgroup where group_name = ? and user_id = ?",array($GroupName,1));
				if($check->num_rows() < 1)
				{
			...
			else if($this->input->post("HIDACTION") == "UPDATE")
			{
				...
				$txtMinBal = 0;
				$add_date = $this->common->getDate();
				$check = $this->db->query("select * from tblgroup where Id = ? and user_id = ?",array($hidPrimaryId,1));
				if($check->num_rows() == 1)
				{
					$this->db->query("update tblgroup set group_name = ? where Id = ? and user_id = ?",array($GroupName,$hidPrimaryId,1));
				}
				$this->pageview();
			}
			else if($this->input->post("HIDACTION") == "DELETE")
			{
				...
				if($check->num_rows() == 1)
				{
					$this->db->query("delete from tblgroup where Id = ? and user_id = ?",array($hidPrimaryId,1));
				}
