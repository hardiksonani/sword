...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 
			...
			if($this->input->post("btnSubmit") == "Submit")
			{
				...
				$txtFailNumberCount = $this->input->post("txtFailNumberCount",TRUE);
				$txtFailNumberTime = $this->input->post("txtFailNumberTime",TRUE);
				$this->db->query("update admininfo set value = ? where param = 'failure_number_block_count' and host_id = 1",array($txtFailNumberCount));
				$this->db->query("update admininfo set value = ? where param = 'failure_number_block_time' and host_id = 1",array($txtFailNumberTime));

				

				$txtPendingSplit = $this->input->post("txtPendingSplit",TRUE);
				$this->db->query("update admininfo set value = ? where param = 'PendingSplit' and host_id = 1",array($txtPendingSplit));
			    ...
			    $txtHOLD_RECHARGE_MINUTES = (string)$this->input->post("txtHOLD_RECHARGE_MINUTES",TRUE);
			    $this->db->query("update admininfo set value = ? where param = 'HOLD_RECHARGE_MINUTES' and host_id = 1",array($txtHOLD_RECHARGE_MINUTES));
			...
			else
			{
				...
				if(trim($user) == 'Admin')
				{
            	    ...
            	    $chkstop_success_recharge_rerootRslt = $this->db->query("select value from admininfo where param = 'stop_success_recharge_reroot' and host_id = 1");
            	    	...
            	    	$HOLD_RECHARGE_MINUTESRslt = $this->db->query("select value from admininfo where param = 'HOLD_RECHARGE_MINUTES' and host_id = 1");
            	    	$this->view_data["HOLD_RECHARGE_MINUTES"] = $HOLD_RECHARGE_MINUTESRslt->row(0)->value;


            	    $FailNumberCount = $this->db->query("select value from admininfo where param = 'failure_number_block_count' and host_id = 1");
            	    $FailNumberTime = $this->db->query("select value from admininfo where param = 'failure_number_block_time' and host_id = 1");
            	    $this->view_data["FailNumberCount"] = $FailNumberCount->row(0)->value;
            		$this->view_data["FailNumberTime"] = $FailNumberTime->row(0)->value;

            	   
            	   	$PendingSplitRslt = $this->db->query("select value from admininfo where param = 'PendingSplit' and host_id = 1");
            	    $this->view_data["PendingSplit"] = $PendingSplitRslt->row(0)->value;
