...
$this->output->set_header("Pragma: no-cache"); 
	...
	public function updateamounts()
	{
	    if(isset($_POST["Id"]) and isset($_POST["amounts"]) and  isset($_POST["amounts_api"]))
	    {
	        ...
	        $amounts_code = trim($this->input->post("code"));
	        $checkoffer = trim($this->input->post("checkoffer"));

	        $result = $this->db->query("update amountwiseapi set amounts = ?,api_id = ?,circle_id = ?,code=?,check_offer = ? where Id = ?",array($amounts,$amounts_api,$amounts_circle,$amounts_code,$checkoffer,$Id));	
	        echo "OK";exit;
	    }
	    else
	    {
...
////////////////////////////////////////////////////////
		...
		order by a.company_id",array($company_id));
		   ...
		   {
			$offer_check_class = "";
		   	if($row->check_offer == "yes")
		   	{
		   		$offer_check_class = "checked";
		   	}
                $str_amounts.='<tr>';
                    $str_amounts.='<td>'.$i.'</td>';
                    ...
                    $str_amounts.='<td style="width:80px;"><input type="text" id="amtcode'.$row->Id.'" value="'.$row->code.'" style="width:80px;" onKeyUp="updataamounts(\''.$row->Id.'\')"></td>';
                    ...
                    $str_amounts.='</span></td>';
 					...
 					$str_amounts.='<td style="width:80px;"><input '.$offer_check_class.' type="checkbox" id="chkcheckoffer'.$row->Id.'" value="'.$row->check_offer.'" style="width:80px;" onClick="updataamounts(\''.$row->Id.'\')"></td>';
