...
class Agent_list_wl extends CI_Controller {
	...
	public function pageview()
	{
		...
		$userrows = $this->db->query("select count(user_id) as total from tblusers where usertype_name = 'WLAgent'");
		
		$total_row = $userrows->row(0)->total;
		$this->load->library('pagination');
		$config['base_url'] = base_url()."_Admin/Agent_list_wl/pageview";
		...
		$this->load->view('_Admin/Agent_list_wl_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
		 	 ...
		 	 if($this->input->post('btnSubmit') == "Search")
			{
				...
				$txtHostName = $this->input->post("txtHostName",TRUE);	
				if($txtHostName != "")							
				{
					$result = $this->Search("HOSTNAME",$txtHostName);	
				}
				else if($txtMOBILENo != "")							
				{
				...
				$this->view_data['txtHostName'] = $txtHostName;
			...
			{							
				...
				if($userinfo->num_rows() == 1)
				{
					if($userinfo->row(0)->status == 0)
					{
						...
						redirect(base_url()."_Admin/Agent_list_wl");
					}
			...
			else if($this->input->post("action") == "delete")
			{
			    ...
			    if($userinfo->num_rows() == 1)
			    {
			      ...
			      if($w1 != 0)
			      {
		            ...
		            redirect(base_url()."_Admin/Agent_list_wl?crypt=".$this->Common_methods->encrypt("MyData"));
			      }
			      else if($w2 != 0)
			      {
			        $this->session->set_flashdata("MESSAGEBOX_MESSAGETYPE","FAILURE");
	...
	public function changeutilitygroup()
	{
		if ($this->session->userdata('aloggedin') != TRUE) 
		{ 
			redirect(base_url().'login'); 
		...
		if($userinfo->num_rows() == 1)
		{
			...
			$this->db->query("update tblusers set utility_group = ? where user_id = ?",array($field,$id));
			echo $field;
		}
	}
