...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			else if($this->input->post('hidactionall') == "settoall")
			{
				...
				if(is_array($chkarr))
				{
					...
					foreach($chkarr as $r)
					{
						...
						if($recharge_info->num_rows() == 1)
						{
							...
							$this->db->query("update tblrecharge set retry = 'dont',updated_by = ? where recharge_id = ?",array($this->session->userdata("adminid"),$recharge_id));
							$this->load->model("Update_methods");
