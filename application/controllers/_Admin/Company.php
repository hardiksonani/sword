...
class Company extends CI_Controller {
    ...
    public function message_test()
    {
        ...
        $this->db->db_debug = TRUE;
        $this->load->model("Sms");
        $message ="Docomo is Down";
        $this->Sms->sendWhatsappNotificationall($message);
    }
...
$this->output->set_header("Pragma: no-cache"); 
	...
	public function ajaxupdate()
	{
        ...
        else if(isset($_POST["company_id"]) and isset($_POST["status"]))
        {
            $company_id = intval(trim($_POST["company_id"]));
            $status = trim(trim($_POST["status"]));



             $this->db->query("update tblcompany set is_enabled = ? where company_id = ?",array($status,$company_id));
