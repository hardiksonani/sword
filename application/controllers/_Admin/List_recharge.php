...
class List_recharge extends CI_Controller {
	...
	public function pageview()
	{
		...
		$this->view_data["userdata"] = $this->Common_methods->getUserForAutocompleteTextBox();
	...
	private function gettablerows($from,$to,$api,$operator,$status,$numid,$start,$perpage,$user_id,$ddlreroot,$ddldb)
	{
		...
		select
				...
				p.businessname as parent_name
							...
							where 
							a.add_date BETWEEN ? and ? and
							if(? != '',a.mobile_no = ?,true) and 
							if(? >= 1,a.company_id =?,true) and
							if(? = 'ALL',true,a.ExecuteBy = ?) and
							if(? = 'ALL',true,a.recharge_status = ?) 
							

							 order by a.recharge_id desc limit ?,?",array($from,$to,$numid,$numid,$operator,$operator,$api,$api,$status,$status,intval($start),intval($perpage)));
		}
