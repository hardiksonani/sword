...
class Lowbalanceret extends CI_Controller {
	...
	public function pageview()
	{
		...
		$config['base_url'] = base_url()."_Admin/Lowbalanceret/pageview";
		...
		where 
		(a.usertype_name = 'Agent' AND a.balance < 100)
		order by a.parentid limit ?,?",array(intval($start_row),intval(50)));
		...
		$this->load->view('_Admin/Lowbalanceret_view',$this->view_data);		
	}
			...
			{							
				...
				if($userinfo->num_rows() == 1)
				{
					if($userinfo->row(0)->status == 0)
					{
						...
						redirect(base_url()."_Admin/Lowbalanceret");
					}
			...
			else if($this->input->post("action") == "delete")
			{
			    ...
			    if($userinfo->num_rows() == 1)
			    {
			      ...
			      else if($w2 != 0)
			      {
			        $this->session->set_flashdata("MESSAGEBOX_MESSAGETYPE","FAILURE");
		            $this->session->set_flashdata("MESSAGEBOX_MESSAGEBODY","To Delete User, Balance Must Be zero");
		            redirect(base_url()."_Admin/Lowbalanceret?crypt=".$this->Common_methods->encrypt("MyData"));
			      }
			      else
			      {
