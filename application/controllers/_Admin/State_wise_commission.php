...
class State_wise_commission extends CI_Controller {
	...
	public function getresult()
	{
		...
		$str = '<div>Commission Structure of   '.$userinfo->row(0)->businessname. ' For '.$company_info->row(0)->company_name.'</div>
		<input type="hidden" id="uid" name="uid" value="'.$user_id.'">
		<input type="hidden" id="cid" name="cid" value="'.$company_id.'">
    	...
    	$i = 0;
		$result_circle = $this->db->query("SELECT a.circleMasterId,a.circleName,IFNULL(b.commission,0) as commission,b.commission_type FROM freecharge_circlemaster a left join statewise_commission b on a.circleMasterId = b.circle_id and b.company_id = ? and b.user_id = ? order by a.circleName",array($company_id,$user_id));
	...
	function ChangeCommission()
	{
		...
		if($companyinfo->num_rows() == 1)
		{
			...
			$rslt = $this->db->query("select * from statewise_commission where circle_id = ? and company_id = ? and user_id = ?",array($circle_id,$company_id,$user_id));
			if($rslt->num_rows() > 0)
			{

				$insertgroupapi = $this->db->query("update statewise_commission set commission=?,commission_type=? where circle_id = ? and company_id = ? and user_id = ?",array($comm,$comtype,$circle_id,$company_id,$user_id));

				echo "OK";exit;
			}
			else
			{
				$this->db->query("delete from statewise_commission  where circle_id = ? and company_id = ? and user_id = ?",array($circle_id,$company_id,$user_id));
				$this->db->query("insert into statewise_commission(company_id,commission,commission_type,circle_id,add_date,ipaddress,user_id) values(?,?,?,?,?,?,?)",array($company_id,$comm,$comtype,$circle_id,$this->common->getDate(),$this->common->getRealIpAddr(),$user_id));
				echo "OK";exit;
			}	
