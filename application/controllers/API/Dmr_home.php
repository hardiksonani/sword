	...
	{	
		...
		{ 
			...
			if($mt_rslt->num_rows() == 1)
			{
				...
				if($mtaccess != '1')
				{
					redirect(base_url()."API/recharge_home?crypt=".$this->Common_methods->encrypt("Mydata"));
				}
			}
			...
			if(trim($user) == 'APIUSER')
			{
										...
										if(isset($jsonarr->status) and isset($jsonarr->statuscode))
										{
											...
											if($status == "0")
											{
												...
												if($is_verified == "0")
												{
    												...
    												$this->session->set_userdata("MTApiId",$this->Common_methods->encrypt($user_id));
    												redirect(base_url()."API/dmr_validate_sender?crypt=".$this->Common_methods->encrypt("getcustomerinfo"));
												}
												else
												{
    												...
    												$this->session->set_userdata("MTSENDERMOBILE",$this->Common_methods->encrypt($txtNumber));
    												$this->session->set_userdata("MTApiId",$this->Common_methods->encrypt($user_id));
    												redirect(base_url()."API/dmr_dashboard?crypt=".$this->Common_methods->encrypt("getcustomerinfo"));
												}
											...
											else
											{
													...
													redirect(base_url()."API/dmr_sender_registration?crypt=".$this->Common_methods->encrypt("MyData"));	
											}
										}
										else
										{
												...
												redirect(base_url()."API/dmr_home?crypt=".$this->Common_methods->encrypt("MyData"));
										}
				...
				{		
						...
						$this->load->view('API/dmr_home_view',$this->view_data);
				}
			} 
		}
	}
