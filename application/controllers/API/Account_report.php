...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$rsltcredit = $this->db->query("select IFNULL(Sum(credit_amount),0) as total from tblewallet where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ?",array($user_id,$from_date,$to_date));
				$rsltdebit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from tblewallet where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($user_id,$from_date,$to_date));
				...
				$this->load->view('API/account_report_view',$this->view_data);		
			}					
			else
			{
