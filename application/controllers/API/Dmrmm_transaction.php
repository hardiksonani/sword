...
class Dmrmm_transaction extends CI_Controller {
	...
	{	
		...
		{ 
			...
			$MTApiId = $this->session->userdata("MT_USER_ID");
			$user_id = $this->session->userdata("ApiId");
			
			
			if($MTLOGIN == true and $MTApiId == $user_id and strlen($MTSENDERMOBILE) == 10)
			{
			//	$mt_rslt = $this->db->query("select mt_access from tblusers where user_id = ?",array($this->session->userdata("ApiId") ));
			$mt_rslt = 	$userinfo = $this->db->query("select user_id,businessname,username,status,usertype_name,mobile_no,mt_access,txn_password from tblusers where user_id = ?",array($user_id));
				...
				if($mt_rslt->num_rows() == 1)
				{
						...
						if(trim($user) == 'APIUSER')
						{
						    ...
						    if($rsltcommon->num_rows() == 1)
						    {
        						...
        						$this->session->set_flashdata("MESSAGEBOX","Service Temporarily Down");
									...
									redirect(base_url()."API/dmrmm_home?crypt=".$this->Common_methods->encrypt("getcustomerinfo"));
						    	}
						    }
								   ...
								   if(ctype_digit($txtNumber))
									...
									//	echo $txtRemitterId.'   '.$txtBeneId."<br>";
											...
											if($checkbeneexist->num_rows() > 0)
											{
												...
												$this->load->view("API/dmrmm_transaction_view",$this->view_data);
											...
											else
											{
												...
												redirect(base_url()."API/dmrmm_home?crypt=".$this->Common_methods->encrypt("MyData"));	
											}	
										}
										else if(isset($_POST["crypt_d4"]) and isset($_POST["crypt_d5"]) and isset($_POST["crypt_d6"]) and isset($_POST["formaction"]))
										{
											...
											if($txtTpin != $userinfo->row(0)->txn_password)
											{
    											...
    											if($checkbeneexist->num_rows() > 0)
    											{
    												...
    												$this->view_data["benedata"] = $checkbeneexist;
    												$this->view_data["remitter_id"] = $this->Common_methods->encrypt($txtRemitterId);
    												$this->view_data["bene_id"] = $this->Common_methods->encrypt($txtBeneId);
    												$this->view_data["transtype"] = $this->Common_methods->encrypt($txtTransType);
    												$this->view_data["message"] = $error_message;
    												$this->load->view("API/dmrmm_transaction_view",$this->view_data);
											    ...
											    if($formaction == "CNFTXN")
    											{
    												...
    												if($balance >= ($txtAmount + 20))
    												{
    													...
    													if($checkbeneexist->num_rows() > 0)
    													{
    														...
    														$this->load->view("API/dmrmm_cnftransaction_view",$this->view_data);
					...
					else
					{
						...
						$this->session->set_userdata("MTApiId","");
						$this->session->set_flashdata("MESSAGEBOX_MESSAGETYPE","FAILURE");
	...
	public function getAccountvalidate()
	{
		...
		$MTApiId = $this->Common_methods->decrypt($this->session->userdata("MTApiId"));
		if(isset($_POST["bid"]))
		{
