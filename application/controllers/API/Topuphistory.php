...
class Topuphistory extends CI_Controller {
	...
	//	echo $start_row;exit;
		...
		$config['base_url'] = base_url()."API/Topuphistory/pageview";
		...
		$this->load->view('API/topuphistory_view',$this->view_data);			
	}
		...
		if(isset($_GET["from"]) and isset($_GET["to"]))
		{
			...
			$data = array();
				...
				left join tblusers p on cr.parentid = p.user_id
				 ...
				 Date(a.add_date) <= ?
				 order by Id";
		$rslt = $this->db->query($str_query,array($this->session->userdata("ApiId"),$from,$to));
