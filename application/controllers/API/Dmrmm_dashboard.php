...
class Dmrmm_dashboard extends CI_Controller {
	...
	public function index() 
	{
		...
		/*$userinfo = $this->db->query("select mt_accept,mt_transo from tblusers where user_id = ?",array($this->session->userdata("ApiId")));
		if($userinfo->row(0)->mt_transo != 'yes')
		{
			redirect(base_url()."API/recharge_home");
		}
		...
		if($this->input->post("txtMobileNO"))
		{
				...
				if(isset($jsonobj->message) and isset($jsonobj->status) and isset($jsonobj->statuscode))
				{
					...
					if($status == "0" and $statuscode == "37")
					{
						...
						if(isset($jsonobj1->message) and isset($jsonobj1->status) and isset($jsonobj1->statuscode))
						{
							...
							if($status1 == "0" and  $statuscode1 == "321")
							{
								...
								redirect(base_url()."API/validate_sender?crypt=".$this->Common_methods->encrypt("dmr2_dashboard"));
							}
						}
					}
					else if($status == "0")
					{
							...
							$this->load->view("API/dmrmm_dashboard_view",$this->view_data);
		...
		else
		{
                    ...
                    else
                    {
    					...
    					if(isset($jsonobj->message) and isset($jsonobj->status) and isset($jsonobj->statuscode))
    					{
    						...
    						else if($status == "0")
    						{
    								...
    								$this->view_data["data"] = $data;
    								$this->load->view("API/dmr2_dashboard_view",$this->view_data);
			...
			{
				redirect(base_url()."API/recharge_home?idstr=".$this->Common_methods->encrypt("Ravikant"));
			}
