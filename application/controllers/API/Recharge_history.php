	...
	//echo "asdfsdf";exit;
		...
		else
		{
            ...
            $rslt_sp = $this->db->query("SELECT 
    						...
    						a.add_date < ?  and a.user_id = ? and a.recharge_status = 'Pending'",array($from,$to,$date,$this->session->userdata("ApiId")));
		...
		$this->view_data['result_all'] = $this->get_recharge($from,$to,$this->session->userdata("ApiId"),$start_row,$per_page,$word,$ddldb,$ddlstatus,$ddloperator);
...
*/
	...
	private function get_recharge($start_date,$end_date,$user_id,$startrow,$perpage,$word,$ddldb,$ddlstatus,$ddloperator)
	{
		...
		if($ddldb == "ARCHIVE")
		{
				...
				$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("ApiId"),$ddlstatus,$ddlstatus,$ddloperator,$ddloperator,$word,$word,intval($startrow),intval($perpage)));
