...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			if($this->input->post('btnSearch') == "Search")
			{
				...
				if($ddldb == "ARCHIVE")
				{
    				...
    				{
    				    $this->view_data['result_all'] = $this->db->query("SELECT a.retry,a.API,a.Id,a.order_id,a.add_date,a.edit_date,a.user_id,a.Charge_type,a.charge_value,a.Charge_Amount,a.RemiterMobile,
    a.debit_amount,a.credit_amount,a.BeneficiaryId,a.AccountNumber,
    a.IFSC,a.Amount,a.Status,a.debited,a.balance,a.mode,
    a.RESP_statuscode,a.RESP_opr_id,a.RESP_status,a.RESP_name,a.row_lock,
    ...
    left join tblusers b on a.user_id = b.user_id
     ...
     if(? != '',a.AccountNumber = ?, true) 
     order by Id desc",array($user_id,$from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo));
    				}
    				else
    				{
    				    $this->view_data['result_all'] = $this->db->query("SELECT a.retry,a.API,a.Id,a.order_id,a.add_date,a.edit_date,a.user_id,a.Charge_type,a.charge_value,a.Charge_Amount,a.RemiterMobile,
    ...
    left join tblusers b on a.user_id = b.user_id
     ...
     if(? != '',a.RemiterMobile = ?, true) and 
     if(? != '',a.AccountNumber = ?, true) 
     order by Id desc limit 50",array($user_id,$from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo));    
    				}
				}
				else
				{
    ...
    left join tblusers b on a.user_id = b.user_id
     ...
     order by Id desc limit 50",array($user_id,$from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo));    
				...
				$this->load->view('API/dmr_report_view',$this->view_data);		
			}	
...
FROM `mt3_transfer` a
left join tblusers b on a.user_id = b.user_id
 where Date(a.add_date) >= ? and Date(a.add_date) <= ? and a.user_id = ? order by Id desc limit 50",array($date,$date,$user_id));
