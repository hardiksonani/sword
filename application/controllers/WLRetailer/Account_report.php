...
class Account_report extends CI_Controller {
	...
	public function index()  
	{
			...
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$rsltcredit = $this->db->query("select IFNULL(Sum(credit_amount),0) as total from WLtblewallet where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ?",array($user_id,$from_date,$to_date));
				$rsltdebit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from WLtblewallet where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($user_id,$from_date,$to_date));
				...
				$this->load->view('WLRetailer/account_report_view',$this->view_data);		
			}					
			else
			{
