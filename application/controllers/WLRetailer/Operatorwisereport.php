...
class Operatorwisereport extends CI_Controller {
	...
	{	
		...
		$this->view_data['result_all'] = $this->get_recharge($from,$to,$this->session->userdata("WLAgentId"));
		$this->view_data['message'] =$this->msg;
		$this->load->view('WLRetailer/operatorwisereport_view',$this->view_data);			
	...
	private function get_recharge($start_date,$end_date,$user_id)
	{
			...
			$str_query = "select count(recharge_id) as totalcount, Sum(amount) as Total,Sum(commission_amount) as Commission,(select company_name from tblcompany where tblcompany.company_id = WLtblrecharge.company_id) as company_name from WLtblrecharge where Date(add_date) >=? and Date(add_date) <=? and WLtblrecharge.user_id = ? and  WLtblrecharge.recharge_status = 'Success' group by company_id";
		$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("WLAgentId")));
		return $result;
