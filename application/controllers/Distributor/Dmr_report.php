...
order by Id desc ",array($user_id,$from,$to,$ddluser,$ddluser));
			...
			$this->load->view('Distributor/dmr_report_view',$this->view_data);								
		}
		else 
		{ 						
...
left join tblusers b on a.user_id = b.user_id
left join dezire_banklist bank on a.bank_id = bank.Id
where b.parentid = ? and Date(a.add_date) >= ? and Date(a.add_date) <= ? order by Id desc",array($user_id,$from,$to));
					$this->view_data['message'] =$this->msg;
					$this->view_data['from'] =$from;
					$this->view_data['to'] =$to;
					$this->view_data['type'] =$status;
