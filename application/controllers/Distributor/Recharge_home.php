...
class Recharge_home extends CI_Controller 
{
    ...
    public function ResetPassword()
    {
    	...
    	{
    		$child_id = $this->input->get("userid");
    		$userrslt = $this->db->query("select user_id,username,mobile_no,businessname from tblusers where user_id = ? and parentid = ?",array($child_id,$user_id));
    		if($userrslt->num_rows() == 1)
    		{
    			$new_password = $this->common->GetPassword();
    			$username = $userrslt->row(0)->username;
    			$mobile_no = $userrslt->row(0)->mobile_no;
    			$name = $userrslt->row(0)->businessname;
    			$this->db->query("update tblusers set password = ? where user_id = ? and parentid = ?",array($new_password,$child_id,$user_id));
    			$emailid = "";


    			$pwdresetlock_check = $this->db->query("select IFNULL(count(Id),0) as totalcount from password_reset_lock where user_id = ? and add_date = ?",array($child_id,$this->common->getMySqlDate()));
    			if($pwdresetlock_check->row(0)->totalcount < 3 )
    			{
    				$insert_pwd_reset_lock = $this->db->query("insert into password_reset_lock(user_id,add_date,ipaddress) values(?,?,?)",array($child_id,$this->common->getMySqlDate(),$this->common->getRealIpAddr()));



	    			$this->load->model("Sms");
	    			$this->Sms->passwordreset($username,$new_password,$mobile_no,$emailid,$name);
    			}
    ...
    <tbody><tr class="operator-showoptcomm">
		...
		select 
		    ...
		    where   a.service_id = ?  order by a.service_id,a.company_name",array($this->session->userdata("DistId"),$service_id));
	...
	public function viewbill()
	{
		...
		{
			$userinfo = $this->db->query("select * from tblusers where user_id = ?",array($this->session->userdata("DistId")));
...
//echo $this->session->userdata("DistId");exit;
				$lasttxns = $this->db->query("select a.recharge_id,a.mobile_no,a.amount,a.operator_id,a.recharge_status,a.add_date,b.company_name from tblrecharge a
                    left join tblcompany b on a.company_id = b.company_id where a.user_id = ? and b.service_id = ? order by a.recharge_id desc limit 10",array($this->session->userdata("DistId"),intval($service_id)));
...
txtAmount: 1500
	   ...
	   // ini_set('display_errors',1);
	    $user_info = $this->db->query("select * from tblusers where user_id = ?",array($this->session->userdata("DistId")));
	    $mcode="";
	    $company_id = 137;
	    $Amount = 1500;
	    $Mobile = "500001373305";
	...
	{	
		...
		$this->output->set_header("Pragma: no-cache"); 
			...
			{		
				...
				$this->load->view('Distributor/recharge_home_view',$this->view_data);
			}
	...
	{
		$mob = $_GET["mob"];
		$rslt = $this->db->query("select custname from tblrecharge where mobile_no = ? and user_id = ? order by recharge_id desc limit 1",array($mob,$this->session->userdata('DistId')));
		if($rslt->num_rows() > 0)
		{
