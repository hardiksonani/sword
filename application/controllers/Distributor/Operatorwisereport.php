...
class Operatorwisereport extends CI_Controller {
	...
	{	
		...
		$this->view_data['result_all'] = $this->get_recharge($from,$to,$this->session->userdata("DistId"));
		$this->view_data['message'] =$this->msg;
		$this->load->view('Distributor/operatorwisereport_view',$this->view_data);			
...
$this->output->set_header("Pragma: no-cache"); 
	...
	private function get_recharge($start_date,$end_date,$user_id)
	{
		...
		$result = $this->db->query("
				select a.user_id,b.businessname,b.mobile_no,
				IFNULL(Sum(a.amount),0) as amount,
				IFNULL(Sum(a.commission_amount),0) as commission_amount,
				count(a.recharge_id) as total_txn,
				IFNULL(Sum(a.DComm),0) as DComm 
				from tblrecharge a 
			...
			",array($start_date,$end_date,$this->session->userdata("DistId")));
