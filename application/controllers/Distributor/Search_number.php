...
class Search_number extends CI_Controller {
	...
	{	
		...
		$this->view_data['result_all'] = $this->get_recharge($this->session->userdata("DistId"),$word);
		$this->view_data['message'] =$this->msg;
		$this->load->view('Distributor/search_number_view',$this->view_data);			
...
$this->output->set_header("Pragma: no-cache"); 
	...
	private function get_recharge($user_id,$word,$ddldb= "LIVE")
	{
		...
		if($ddldb == "ARCHIVE")
		{
				...
				$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("DistId"),$ddlstatus,$ddlstatus,$ddloperator,$ddloperator,$word,$word,intval($startrow),intval($perpage)));
		...
		else
		{
			$str_query ="
				...
				b.mobile_no = ?
				order by a.Id desc ";		
				$result = $this->db->query($str_query,array($this->session->userdata("DistId"),$word));
				return $result;
