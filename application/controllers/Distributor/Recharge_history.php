...
class Recharge_history extends CI_Controller {
	...
	//echo "asdfsdf";exit;
		...
		$user = $this->session->userdata("DistId");
		...
		$config['base_url'] = base_url()."Distributor/recharge_history/commonfunction";
        ...
        $this->db->reconnect();
			...
			$rslt_sf = $this->db->query("SELECT 
										...
										b.parentid = ? and
										if(? > 0,a.user_id = ?,true)
										 group by a.recharge_status ",array($from,$to,$this->session->userdata("DistId"),$ddluser,$ddluser));
		...
		$this->view_data['result_all'] = $this->get_recharge($from,$to,$this->session->userdata("DistId"),$start_row,$per_page,$word,$ddldb,$ddlstatus,$ddloperator,$ddluser);
		$this->view_data['message'] =$this->msg;
		$this->load->view('Distributor/recharge_history_view',$this->view_data);			
	...
	private function get_recharge($start_date,$end_date,$user_id,$startrow,$perpage,$word,$ddldb,$ddlstatus,$ddloperator,$ddluser)
	{
		...
		{ 
			...
			$str_query ="
				...
				Date(b.add_date)<= ? and 
				u.parentid = ?  and
				if(? > 0,b.user_id = ?,true) and
				if(? != 'ALL',b.recharge_status = ?,true) and
				if(? != 'ALL',b.company_id = ?,true) and
				if(? != '',b.mobile_no = ?,true) 
				order by a.Id desc";		
				$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("DistId"),$ddluser,$ddluser,$ddlstatus,$ddlstatus,$ddloperator,$ddloperator,$word,$word));
				return $result;
	...
	private function get_recharge_all($start_date,$end_date,$user_id,$ddldb)
	{
		...
		tblcompany.company_id=tblrecharge.company_id and Date(tblrecharge.add_date)>=? and Date(tblrecharge.add_date)<= ? and tblrecharge.user_id=?  order by tblrecharge.recharge_id";		
			$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("DistId")));
			return $result;
		}
		else
		{
