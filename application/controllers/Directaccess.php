...
$this->output->set_header("Pragma: no-cache"); 
		...
		else 		
		{
			...
			where a.user_id = ?",array($user_id));
					...
					else if($rsltuserinfo->row(0)->usertype_name == "WLAgent")	
					{
						$data = array(
							'WLAgentId' => $rsltuserinfo->row(0)->user_id,
							'WLAgentParentId' => $rsltuserinfo->row(0)->parentid,
							'WLAgentLoggedIn' => true,
							'WLAgentUserType' => $rsltuserinfo->row(0)->usertype_name,
							'WLAgentUserName' => $rsltuserinfo->row(0)->username,
							'WLAgentBusinessName' => $rsltuserinfo->row(0)->businessname,
							'WLAgentSchemeId'=>$rsltuserinfo->row(0)->scheme_id,
							'WLAgentGroupName'=>$rsltuserinfo->row(0)->group_name,
							'WLAgentMT_Access'=>$rsltuserinfo->row(0)->mt_access,
							'WLAgentMobile'=>$rsltuserinfo->row(0)->mobile_no,
							'WLAgentBalance'=>$rsltuserinfo->row(0)->balance,
							'WLAgentpostal_address'=>$rsltuserinfo->row(0)->postal_address,
							'WLAgentpincode'=>$rsltuserinfo->row(0)->pincode,
							'WLAgentaadhar_number'=>$rsltuserinfo->row(0)->aadhar_number,
							'WLAgentpan_no'=>$rsltuserinfo->row(0)->pan_no,
							'WLAgentgst_no'=>$rsltuserinfo->row(0)->gst_no,
							'WLAgentemailid'=>$rsltuserinfo->row(0)->emailid,
							'AdminId'=>1,
							'ReadOnly'=>false,
							'Redirect'=>base_url()."WLRetailer/Account_report",
							);
							$this->session->set_userdata($data);
							redirect(base_url()."WLRetailer/Account_report");
