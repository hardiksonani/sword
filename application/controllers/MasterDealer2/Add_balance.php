...
class Add_balance extends CI_Controller {
	...
	public function index() 
	{
		if(isset($_POST["ddlaction"]) and isset($_POST["txtAmount"]) and isset($_POST["txtRemark"]) ) 	
		{
			...
			$userinfo = $this->db->query("select user_id,businessname,username,usertype_name from tblusers where user_id = ? and parentid = ?",array($hidid,$this->session->userdata("MdId")));
			if($userinfo->num_rows() == 1)
			{
				if($ddlaction == "ADD")
				{
					$description = $this->session->userdata("MdBusinessName")." To ".$userinfo->row(0)->businessname;
					$payment_type = "CASH";
				...
				{
					$cr_user_id = $this->session->userdata("MdId");
					$dr_user_id = $userinfo->row(0)->user_id;
					$description = $userinfo->row(0)->businessname." To ".$this->session->userdata("MdBusinessName");
					$payment_type = "CASH";
		...
		else
		{
			if(isset($_GET["encrid"]))
			{
				...
				$userinfo = $this->db->query("select user_id,businessname,username,usertype_name,add_date from tblusers where user_id = ? and parentid = ?",array($encrid,$this->session->userdata("MdId")));
				if($userinfo->num_rows() == 1)
				{
					$this->view_data["message"] = "";
					$this->view_data["userinfo"] = $userinfo;
					$this->load->view("MasterDealer/add_balance_view",$this->view_data);
				}
