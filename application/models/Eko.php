                					...
                					if($ewallet_id > 10)
                					{
                						...
                						else if($sub_txn_type == "REMITTANCE")
                						{
                									...
                									if($reslut2 == true)
                									{
                										...
                										$rslt_updtrec = $this->db->query("update masterpa_archive.mt3_transfer set reverted='yes',balance=CONCAT_WS(',',balance,?), ewallet_id = CONCAT_WS(',',ewallet_id,?),credit_amount = ? where Id = ?",array($current_balance2,$ewallet_id2,$totaldebit_amount,$transaction_id));	
                										return true;
                									}
                									else
                									{
                										...
                										$rslt_updtrec = $this->db->query("update masterpa_archive.mt3_transfer set reverted='yes',balance=CONCAT_WS(',',balance,?), ewallet_id = CONCAT_WS(',',ewallet_id,?),credit_amount = ? where Id = ?",array($current_balance,$ewallet_id,$dr_amount,$transaction_id));	
                										return false;
                									}
