...
public function getCommissionInfo($company_id,$userinfo,$Amount,$state_id = 0,$is_debug = false)
{
	    $user_id = $userinfo->row(0)->user_id;
		...
		//if(false)
		{
		    ...
		    else if($rslt->row(0)->commission_type == "STATE_WISE" and $state_id > 0 and $userinfo->row(0)->usertype_name == 'APIUSER')
		    {

		    	if($is_debug == true)
				{
					echo "Inside statesise commission <br>";
					echo "company id : ".$company_id."   stateid : ".$state_id."   userid : ".$userinfo->row(0)->user_id."<br>";
				}


		    		$rsltstatecommission = $this->db->query("SELECT commission,commission_type FROM statewise_commission where company_id = ? and circle_id = ? and user_id = ?",array($company_id,$state_id,$userinfo->row(0)->user_id));


		    		if($is_debug == true)
					{
						print_r($rsltstatecommission->result());
						echo "<br>";
					}


		    		if($rsltstatecommission->num_rows() == 1)
		    		{
		    			$resparr = array(
    					"Commission_Type"=>$rsltstatecommission->row(0)->commission_type,
    					"Commission_Per"=>$rsltstatecommission->row(0)->commission,
    					"TYPE"=>"ENTITY",
    					);
    					return $resparr;	
		    		}
...
public function ProcessRecharge($user_info,$circle_code,$company_id,$Amount,$Mobile,$recharge_type,$service_id,$rechargeBy,$order_id,$is_check_api = false)
{
	...
	///check recharge stopped or not
	$recharge_stop_info = $this->db->query("select value from admininfo where param = 'RECHARGE_STOP'");
	if($recharge_stop_info->num_rows() == 1)
	{
		if($recharge_stop_info->row(0)->value == "yes")
		{
			$resp_status = "Failure";
	        $resp_message = "System Under Maintanance";
	        $this->custom_response(0,$Mobile,$Amount,$resp_status,$resp_message,$order_id,$response_type,$rechargeBy,$user_id);
		}
	}
	...
	$mobile_number_min_length = $company_info->row(0)->mobile_number_min_length;
	$mobile_number_max_length = $company_info->row(0)->mobile_number_max_length;
	...
	if($is_enabled == "no")
	{
		$order_id = "";
		$resp_status = "Failure";
        $resp_message = "Operator Downtime. Please try after some time";
        $this->custom_response(0,$Mobile,$Amount,$resp_status,$resp_message,$order_id,$response_type,$rechargeBy,$user_id);
	}

	if($mobile_number_min_length > 0 and strlen($Mobile) < $mobile_number_min_length)
	{
		$order_id = "";
		$resp_status = "Failure";
        $resp_message = "Invalid Mobile Number";
        $this->custom_response(0,$Mobile,$Amount,$resp_status,$resp_message,$order_id,$response_type,$rechargeBy,$user_id);
	}
	if($mobile_number_max_length > 0 and strlen($Mobile) > $mobile_number_max_length)
	{
		$order_id = "";
		$resp_status = "Failure";
        $resp_message = "Invalid Mobile Number";
    ...
    {
       $rsltstatea = $this->db->query("select * from freecharge_circlemaster where circleMasterId = ?",array($circle_id));  
       if($rsltstatea->num_rows() == 1)
       {
       		echo "Circle Name : ".$rsltstatea ->row(0)->circleName;
       		echo "<hr>";
       } 
    }
...
where a.company_id = ? and api.enable_recharge = 'yes' and a.status = 'active' and a.api_id > 0 order by a.priority",array($company_id));
    ...
    else
    {
        ...
        foreach($ApiInfo->result() as $apirw)
        {
		    ...
		    {
			   $foundoffer = $this->checkoffer($Mobile,$company_info->row(0)->mcode,$Amount);
...
//////////////////////////////////////////////////////
    ...
    if($userwise_switching->num_rows() == 1)
    {
	    ...
	    if($this->checkoperatorlimit($user_id,$company_id) == false)
    	{
    	    $msg =  "0000#Failure#ERROR::Operator Limit Over";
			return $this->custom_response(0,$Mobile,$Amount,"Failure","Operator Limit Over",$order_id,$response_type,$rechargeBy,$user_id);
    	}
   ...
   $ApiInfo = $this->db->query("select api_name from api_configuration where Id = ?",array($api_id));
	...
	if($groupinfo->num_rows() == 1)
	{
		...
		if($service == 0)
		{
			...
			//return $this->errorresponse($msg,$rechargeBy);
			return $this->custom_response(0,$Mobile,$Amount,"Failure","Your Service Is Deactivated. Please Contact Administration",$order_id,$response_type,$rechargeBy,$user_id);
		}
		
		if($Amount < $minAmt)
		{
			$msg =  "Minimum Amount For This Operator is : ".$minAmt;
			return $this->custom_response(0,$Mobile,$Amount,"Failure",$msg,$order_id,$response_type,$rechargeBy,$user_id);
			//return $this->errorresponse($msg,$rechargeBy);	
		}
		if($Amount > $maxAmt)
		{
		...
		if($this->CheckTimeInterval($Mobile,$Amount) == false)
		{
			$msg =  "0000#Failure#ERROR::Same Number Same Amount";
			return $this->custom_response(0,$Mobile,$Amount,"Failure",$msg,$order_id,$response_type,$rechargeBy,$user_id);
			//return $this->errorresponse($msg,$rechargeBy);
		}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					////////////////////////////////////////////////////////////////////////////////////////////////////////////
							...
							$commission_arr = $this->getCommissionInfo($company_id,$userinfo,$Amount,$state_id,$is_check_api);


							if($is_check_api == true)
						     {
						     	echo "StateId : ".$state_id."<br>";
					...
					if(true)
						{
							...
							$str_query = "insert into tblrecharge(
								...
								commission_amount,commission_per,
								ExecuteBy,order_id,state_id,AdminCommPer,AdminComm) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
							...
							$result = $this->db->query($str_query,array(
								...
								$commission_amount,$commission_per,
								$ExecuteBy,$order_id,$state_id,$adminComPer,$adminCom));	
							if($result > 0)
							{
                                ...
                                if($allowed_retry > 0)
                                {
                                    $this->db->query("insert into reroot_count(recharge_id,allowed_retry,recharge_api,retry_api_1,retry_api_2,retry_api_3) values(?,?,?,?,?,?)",
                                                        array($recharge_id,$allowed_retry,$ApiInfo->row(0)->Id,$reroot_api1_id,$reroot_api2_id,$reroot_api3_id));    
                                }
								...
								if(true)
								{
									...
									if($ewid > 1)	
									{
										if($ApiInfo->row(0)->api_name == "HOLD")		
										{
										    $status = 'Pending';
                                            $operator_id = "";
                                           $this->db->query("insert into tblholdtransactions(recharge_id,add_date,mobile_no,amount,company_id,ExecuteBy,status,remark,flat_value) values(?,?,?,?,?,?,?,?,?)",
                                       		array($recharge_id,$this->common->getDate(),$Mobile,$Amount,$company_id,"HOLD","Hold","",0));
                        				    return $this->custom_response($recharge_id,$Mobile,$Amount,$status,$operator_id,$order_id,$response_type,$rechargeBy,$user_id); 
										}
										else if($ApiInfo->row(0)->api_name == "AUTO_SUCCESS")		
										{

											$status = 'Success';
											$permitted_chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
											$Operator_id_last =  substr(str_shuffle($permitted_chars), 0, 6);
                                            $operator_id = "BR0".rand(111,999).$Operator_id_last;
                                            $this->db->query("update tblrecharge set operator_id = ? where recharge_id = ?",array($operator_id,$recharge_id));
...
//******************************************************************************************************************************//							
					...
					{
					    $resp_status = "Failure";
					    $this->custom_response(0,$Mobile,$Amount,$resp_status,"InSufficient Balance",$order_id,$response_type,$rechargeBy,$user_id);
					}
				}	
			else
			{
...
public function get_string_between($string, $start, $end)
	...
	private function CheckTimeInterval($mobile,$amount)
	{
		...
		if($result->num_rows() == 1)
		{
			...
			if( (( strtotime($stime)  - strtotime($etime)) / 60) > 60)
			{return true;}
			else
			{return false;}
		}
