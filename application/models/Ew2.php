...
{	
	...
	public function tblewallet_Payment_CrDrEntry($credit_user_id,$debit_user_id,$amount,$remark,$description,$payment_type,$admin_remark = "")
	{	
		...
		if($payment_id > 0)
		{
			...
			$reslut_debit = $this->db->query("insert into  tblewallet2(user_id,payment_id,transaction_type,remark,description,add_date,debit_amount,balance,ipaddress,payment_type,cr_user_id,dr_user_id,admin_remark)
			values(?,?,?,?,?,?,?,?,?,?,?,?,?)",array($debit_user_id,$payment_id,$transaction_type,$remark,$description,$add_date,$credit_amount,$current_balance_debit_user_id,$ipaddress,$payment_type,$credit_user_id,$debit_user_id,$admin_remark));	
			
			$dr_insert_id = $this->db->insert_id();
			if($dr_insert_id > 1)
			{
					$reslut = $this->db->query("insert into  tblewallet2(user_id,payment_id,transaction_type,remark,description,add_date,credit_amount,balance,ipaddress,payment_type,cr_user_id,dr_user_id,admin_remark)
					values(?,?,?,?,?,?,?,?,?,?,?,?,?)",array($credit_user_id,$payment_id,$transaction_type,$remark,$description,$add_date,$credit_amount,$current_balance_credit_user_id,$ipaddress,$payment_type,$credit_user_id,$debit_user_id,$admin_remark));
			...
			if($dr_user_type == "MasterDealer" or $dr_user_type == "Distributor" or $dr_user_type == "FOS")
			{
				...
				if($balance >= $amount)
				{
					...
					//if(true)
					{
						...
						$this->tblewallet_Payment_CrDrEntry($cr_user_id,$dr_user_id,$amount,$remark,$description,$payment_type);
						$this->load->model("Sms");
