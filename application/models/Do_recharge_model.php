...
private function errorresponse($message,$status,$rechargeBy)
{
    ...
    {
    	$message = "Recharge Submitted Successfully Pl Check Status";
    }
...
public function ProcessRecharge($user_info,$circle_code,$company_id,$Amount,$Mobile,$recharge_type,$service_id,$rechargeBy,$custname,$is_check_api = false,$is_check_commission = false)
{
	...
	if($parentinfo->num_rows() == 1)
	{
	    ...
	    $flatcomm = (($amount * $flatcomm_per)/100);
	}
...
//////////////////////////////////////////////////////
   ...
   {
	   	if($ApiInfo->row(0)->api_name == "STOP" or $ApiInfo->row(0)->api_name == "Circle_wise")
	   {
	   		$request_str = $user_info->row(0)->businessname." | ".$company_name." | ".$Mobile." | ".$Amount." | ";
	   		$resp_message = "Operator Downtime";
	   		$this->db->query("insert into tblreqresp(request,response,add_date,ipaddress,user_id) values(?,?,?,?,?)",array($request_str,$resp_message,$this->common->getDate(),$this->common->getRealIpAddr(),$user_id));
	   		$resp_status = "Failure";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					$md_info = $this->db->query("select user_id,scheme_id,usertype_name from tblusers where user_id = ?",array($dist_info->row(0)->parentid));
					...
					{
						$dr_commission_amount = $commission_amount;
						$dr_commission_per = $commission_per;	

						$md_commission_per =	$dist_commission_per;	
						$md_commission_amount = $dist_commission_amount;

						$dist_commission_per =	$commission_per;
						$dist_commission_amount = $commission_amount ;
					...
					if($userinfo->row(0)->usertype_name == "MasterDealer")
					{
						...
						$md_commission_per =	$commission_per;	
						$md_commission_amount = $commission_amount;
					...
					if(true)
						{
							...
							if($result > 0)
							{
								...
								$dr_amount = $Amount - $dr_commission_amount;	
								$this->load->model("Update_methods");
