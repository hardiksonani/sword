...
{	
	...
	public function tblewallet_Payment_CrDrEntry_AEPS($credit_user_id,$debit_user_id,$amount,$remark,$description,$payment_type,$admin_remark = "")
	{	
	...
	public function tblusers_registration_Entry($parentid,$distributer_name,$postal_address,$pincode,$state_id,$city_id,$contact_person,$mobile_no,$emailid,$usertype_name,$status,$scheme_id,$username,$password,$aadhar,$pan,$gst,$downline_scheme,$downline_scheme2,$BDate,$service_array)
	{
	...
	//    echo "state id ".$state_id."    ".$city_id;exit;
	   ...
	   $host_id = $this->Common_methods->getHostId($this->white->getDomainName());	
		...
		else
		{
			...
			{
			    $fos_id = 0;
				$parentinfo = $this->db->query("select user_id,parentid,usertype_name,mobile_no,status,businessname,downline_scheme,downline_scheme2 from tblusers where user_id = ? and host_id = ?",array(intval($parentid),intval($host_id)));
			
				if($parentinfo->num_rows() == 1)
				{
					
					
					if($parentinfo->row(0)->usertype_name == "FOS")
					{
						
						$fos_id = $parentinfo->row(0)->user_id;
						$fos_parent_id = $parentinfo->row(0)->parentid;
						$parentinfo = $this->db->query("select user_id,usertype_name,mobile_no,status,businessname,downline_scheme,downline_scheme2 from tblusers where user_id = ? and host_id = ?",array(intval($fos_parent_id),intval($host_id)));
						$parentid = $parentinfo->row(0)->user_id;
						...
						if(true)
						{
							if($parent_type == "Distributor")
							{
								if($parent_status == '1')
								{
										...
										$str_query = "insert into  tblusers(parentid,businessname,state_id,city_id,mobile_no,usertype_name,add_date,ipaddress,status,scheme_id,username,password,txn_password,fos_id,host_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
										$rlst = $this->db->query($str_query,array($parentid,$distributer_name,
										$state_id,$city_id,$mobile_no,$usertype_name,
										$add_date,$ipaddress,"1",53,$username,$password,$password,$fos_id,$host_id));		
										...
										if($reg_id > 1)
										{
                                            ...
                                            $this->addservice($reg_id,$service_array);
                                         	...
                                         	$this->setCommission($reg_id,$usertype_name);
											$this->load->model("Sms");
					...
					{
						$resparr = array(
										"message"=>"Commission Group Not Selected For Downline",
										"status"=>1
									);
						return json_encode($resparr);
					}
				...
				if($parentinfo->num_rows() == 1)
				{
					...
					if($parent_type == "MasterDealer")
					{
						if($parent_status == '1')
						{
								...
								$rlst = $this->db->query($str_query,array($parentid,$distributer_name,
								$state_id,$city_id,$mobile_no,$usertype_name,
								$add_date,$ipaddress,"1",52,$username,$password,$password,$downline_scheme2,0,$host_id));		
								$reg_id = $this->db->insert_id();
								if($reg_id > 1)
								{
									$insertinfo = $this->db->query("insert into tblusers_info(user_id,add_date,ipaddress,postal_address,pincode,aadhar_number,pan_no,gst_no,emailid,birthdate) 
	...
	public function setCommission($reg_id,$usertype_name)
	{
			$rslt = $this->db->query("select company_id from tblcompany");
			foreach($rslt->result() as $row)
			{
				$comm = $this->getCommByScheme($row->company_id,$usertype_name);
	...
	public function getCommByScheme($company_id,$usertype_name)
	{
		$rslt = $this->db->query("select * from tblgroupapi where  company_id = ?",array($company_id));
		if($rslt->num_rows() > 0)
		{
			if($usertype_name == "Agent")
			{
				return 	$rslt->row(0)->RetailerComm;	
			}
			else if($usertype_name == "Distributor")
			{
				return 	$rslt->row(0)->DistComm;	
			}
			else if($usertype_name == "MasterDealer")
			{
				return 	$rslt->row(0)->MdComm;	
			}
			else if($usertype_name == "APIUSER")
			{
				return 	$rslt->row(0)->ApiComm;	
			}	
		}
		else
		{
	...
	public function getCreditPaymentDescription($credit_user_id, $debit_user_id,$amount)
	{
		...
		return "Direct Payment By ".$debit_user_info->row(0)->businessname." (".$debit_user_info->row(0)->usertype_name." )  To ".$credit_user_info->row(0)->businessname." (".$credit_user_info->row(0)->usertype_name." )" ;
	}
	public function getRevertPaymentDescription($cr_user_id, $dr_user_id,$amount)
	{
		...
		return "Direct Revert By ".$credit_user_info->row(0)->businessname." (".$credit_user_info->row(0)->usertype_name." )  From ".$debit_user_info->row(0)->businessname." (".$debit_user_info->row(0)->usertype_name." )" ;
	}
	...
	public function set_user_commission($user_id,$scheme_id)
	{
	...
	public function refundOfAcountReportEntry($recharge_id)
	{
		...
		if($rsltrech->num_rows() == 1)
		{
			...
			{
				$commission_amount = $rsltrech->row(0)->DComm;
				$commission_per = $rsltrech->row(0)->DComPer;
			}
			if($usertype_name == "MasterDealer")
			{
				$commission_amount = $rsltrech->row(0)->MdComm;
				$commission_per = $rsltrech->row(0)->MdComPer;
			}
