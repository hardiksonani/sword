...
{	
	...
	{
		$rsltrechargeinfo = $this->db->query("select recharge_id,MdId,MdComm,MdCom_Given,DId,DComm,DCom_Given,recharge_status from tblrecharge where recharge_id = ? and recharge_status = 'Failure'",array($recharge_id));
		if($rsltrechargeinfo->num_rows() == 1)
		{
			$MdId = $rsltrechargeinfo->row(0)->MdId;
			$MdComm = $rsltrechargeinfo->row(0)->MdComm;
