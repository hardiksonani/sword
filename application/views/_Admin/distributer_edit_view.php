						 ...
						 <div class="row row-sm mg-t-20">
									  <div class="col-sm-12 col-lg-12">
										<div class="card shadow-base bd-0">
										  ...
										  <div class="card-body">
                                        <form method="post" action="<?php echo base_url()."_Admin/distributer_edit?crypt=".$this->Common_methods->encrypt("MyData");?>" name="frmdistributer_form1" id="frmdistributer_form1" autocomplete="off">
...
<tr>
    ...
    <td>
        <select id="ddlparent" name="ddlparent" class="text" style="width:300px;height:30px;">
        <option value="1">Select</option>
        <?php
        	$rlstparent = $this->db->query("select user_id,businessname from tblusers where usertype_name='MasterDealer' and host_id = 1");
...
<tr>
    ...
    <td>
        <select style="width:300px;height:30px;" class="text" id="ddlSchDesc" onChange="ChangeAmount()" placeholder="Select Scheme Name.Click on drop down" name="ddlSchDesc">
          ...
          <?php
            $str_query = "select * from tblgroup where groupfor = 'Distributor'";
...
<tr>
	<td> 					
         ...
         <select style="width:300px;" class="form-control-sm" id="ddlDownSchDesc"  name="ddlDownSchDesc"> 					  <option>Select Scheme</option> 					  
         <?php 				
         $str_query = "select Id,group_name from tblgroup where groupfor = 'Agent'"; 						$resultScheme = $this->db->query($str_query);		 						
         foreach($resultScheme->result() as $row) 						
         { 							
         echo "<option value='".$row->Id."'>".$row->group_name."</option>"; 						
         } 						
...
<?php
    ...
    ?>
        <tr>
            ...
            <td>
                <input type="number" id="txttxnLimit<?php echo $cmp->company_id; ?>" name="txttxnLimit<?php echo $cmp->company_id; ?>" class="text" style="color:#000000;font-weight:bold;width:120;" value="<?php echo $cmp->txn_limit; ?>">
            </td>
            <td>
                <input type="number" id="txtAmountLimit<?php echo $cmp->company_id; ?>" name="txtAmountLimit<?php echo $cmp->company_id; ?>" class="text" style="color:#000000;font-weight:bold;width:120;" value="<?php echo $cmp->loadlimit; ?>">
            </td>
...
function changeCommission_all()
{
   ...
   for(i=0;i<struserarr.length;i++)
   {
       ...
       if(jcompany_id > 0)
       {
           params[jcompany_id]= document.getElementById("txtComm"+jcompany_id).value+"@"+document.getElementById("ddlcommtype"+jcompany_id).value+"@"+document.getElementById("ddlapi1"+jcompany_id).value+"@"+document.getElementById("txttxnLimit"+jcompany_id).value+"@"+document.getElementById("txtAmountLimit"+jcompany_id).value;
       }   
   }
   $.ajax({
          type: "POST",
